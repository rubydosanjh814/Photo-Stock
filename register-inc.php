...
$confirmPass=$_POST['ConfirmPassWord'];
   ...
   {
   header("location: register.php?error=passwordDoestMatch");
   exit();
   }
   else
   {
   $sql ="SELECT UserName FROM user_data WHERE UserName=?";
      ...
      else
      {
       ...
       $resultcheck= mysqli_stmt_num_rows($stmt);
        if($resultcheck>0)
          ...
          {
          $sql="INSERT INTO user_data (Name,Email,UserName,PassWord) VALUES(?,?,?,?)";
          $stmt= mysqli_stmt_init($db);
            if(!mysqli_stmt_prepare($stmt,$sql))
            {
            header("Location: register.php?error=sqlerror");
